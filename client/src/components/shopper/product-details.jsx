...
function ProductDetailDialog({ open, setOpen, productDetails }) {
  ...
  const { reviews: allReviews } = useSelector(
    (state) => state.shopProductReview
  );
  const productReviews = productDetails?._id
    ? allReviews?.[productDetails._id] || []
    : [];
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => !val && handleDialogClose()}>
      <DialogContent className="max-w-[90vw] sm:max-w-[80vw] lg:max-w-[70vw] max-h-[90vh] overflow-y-auto">
          ...
          <div className="gap-6">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-muted-foreground">
                ({averageReview.toFixed(1)} from {productReviews.length}{" "}
                reviews)
              </span>
            </div>
            ...
            <div className="overflow-auto max-h-[300px]">
              ...
              <div className="grid gap-6">
                {productReviews.length > 0 ? (
                  productReviews.map((reviewItem, index) => (
